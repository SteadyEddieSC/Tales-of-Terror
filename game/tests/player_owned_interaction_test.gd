...
func _test_player_owned_complete_route() -> void:
	...
	var threshold: Dictionary = coordinator.submit_player_interaction(1, "confirm")
	...
	_expect(
		coordinator.submit_player_interaction(1, "confirm").accepted,
		"resolves the completed threshold choice",
	)
	_expect(coordinator.stage_index == 1, "advances to the council stage")

	_expect(
		coordinator.submit_player_interaction(2, "confirm").waiting_for_players,
	...
	_expect(
		coordinator.submit_player_interaction(3, "confirm").waiting_for_players,
	...
	var wrong_card: Dictionary = coordinator.submit_player_interaction(2, "confirm")
	...
	_expect(coordinator.submit_player_interaction(1, "confirm").accepted, "Seat I plays the card")
	...
	_expect(
		not coordinator.submit_player_interaction(3, "confirm").accepted,
	...
	_expect(coordinator.submit_player_interaction(1, "confirm").accepted, "Seat I attempts the check")
	...
	_expect(
		not coordinator.submit_player_interaction(9, "confirm").accepted,
	...
	_expect(coordinator.submit_player_interaction(2, "confirm").accepted, "an active seat continues")
	...
	_expect(
		coordinator.submit_player_interaction(1, "confirm").waiting_for_players,
	...
	_expect(
		not coordinator.submit_player_interaction(non_actor, "confirm").accepted,
	...
	_expect(
		coordinator.submit_player_interaction(afterlife.owner_seat, "pass").accepted,
		"permits the authored afterlife pass",
	)
	_expect(coordinator.stage_index == 4, "advances to the ending stage")
	_expect(coordinator.submit_player_interaction(1, "confirm").accepted, "resolves the ending")
...
func _test_pause_and_invalid_seat_are_non_mutating() -> void:
	...
	coordinator.submit_player_interaction(1, "confirm")
	...
	var blocked: Dictionary = coordinator.submit_player_interaction(1, "confirm")
	...
	var invalid: Dictionary = coordinator.submit_player_interaction(8, "confirm")
