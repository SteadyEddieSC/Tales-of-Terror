...
func _initialize() -> void:
	...
	for device_id: int in 8:
		_expect(
			seats.join_device(device_id, "guid-%d" % device_id, "Pad %d" % device_id) == device_id,
			"assigns device %d" % device_id
		)
	...
	_expect(
		seats.reconnect_device(12, "guid-3", "Pad 3 reconnected") == 3,
		"reclaims reservation by identity"
	)
	var reclaimed: Dictionary = seats.get_seats()[3]
	_expect(
		reclaimed.device_id == 12 and reclaimed.state == SeatManager.SeatState.ACTIVE,
		"updates device ID without moving seat"
	)
	...
	_expect(
		twins.reconnect_device(7, "shared-guid", "Twin Pad") == -1,
		"does not guess between ambiguous reservations"
	)
	_expect(
		twins.reconnect_device(1, "shared-guid", "Twin Pad") == 1,
		"prefers previous device ID for identical controllers"
	)
