...
func _initialize() -> void:
	...
	var neighboring_pairs: Array[Vector2i] = [
		...
		Vector2i(2, 4), Vector2i(3, 5), Vector2i(4, 5), Vector2i(4, 6),
		Vector2i(5, 7), Vector2i(6, 7),
	]
	for pair: Vector2i in neighboring_pairs:
		var luminance_delta: float = absf(tokens.player_colors[pair.x].get_luminance() - tokens.player_colors[pair.y].get_luminance())
		_expect(luminance_delta >= 0.12, "separates neighboring seats %d and %d by value" % [pair.x + 1, pair.y + 1])
	...
	_expect(safe_overlay.mouse_filter == Control.MOUSE_FILTER_IGNORE, "keeps the safe-area frame input-transparent")
