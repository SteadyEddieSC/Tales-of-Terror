...
func _initialize() -> void:
	...
	if _failures == 0:
		print(
			"Companion simulation passed: %d deterministic 1–8 client sequences" % _sequence_count
		)
	quit(_failures)
...
func _run_sequence(seed: int, seat_count: int) -> void:
	...
	_expect(
		(
			first.bridge.create_room("room_sim", "GHST27").accepted
			and second.bridge.create_room("room_sim", "GHST27").accepted
		),
		"creates deterministic simulated rooms"
	)
	...
	_expect(
		first.bridge.public_view() == second.bridge.public_view(),
		"replays public view for seed/count"
	)
	_expect(
		first.bridge.diagnostics() == second.bridge.diagnostics(),
		"replays sanitized diagnostics for seed/count"
	)
	...
	for index: int in seat_count:
		...
		_expect(
			private_first == private_second and private_first.authorized_seat == index + 1,
			"replays stable-seat filtered view"
		)
		first.transport.disconnect_client(client_id)
		_expect(
			first.transport.resume_client(client_id, index + 1).accepted,
			"reconnects only the same stable seat"
		)
		_expect(
			(
				first.bridge.seat_view_for_client(client_id).social_private
				== private_first.social_private
			),
			"preserves private state on reconnect"
		)
	_expect(first.roles.privacy_report().passed, "passes recursive social privacy evaluation")
	_expect(
		not "capability" in JSON.stringify(first.bridge.diagnostics()).to_lower(),
		"keeps resume capabilities out of diagnostics"
	)
	_sequence_count += 1
