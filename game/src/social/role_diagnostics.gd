...
func _assignment_page(diagnostics: Dictionary) -> String:
	var lines := PackedStringArray([
		"CONTENT %s v%d  •  MODE %s  •  SNAPSHOT v%d" % [diagnostics.scenario_id, diagnostics.scenario_version, diagnostics.mode_id, RoleSession.SNAPSHOT_VERSION],
		"ROLE RNG seed=%d state=%d counter=%d  •  revision=%d" % [diagnostics.rng.initial_seed, diagnostics.rng.state, diagnostics.rng.counter, diagnostics.revision],
		"Assignment plan and bounded audit are complete spoiler state.",
	])
	for entry: Dictionary in diagnostics.audit_history.slice(maxi(0, diagnostics.audit_history.size() - 8)):
		lines.append("#%d r%d %s  %s" % [entry.sequence, entry.revision, entry.type, JSON.stringify(entry.private)])
	return "\n".join(lines)
...
func _seat_page(diagnostics: Dictionary) -> String:
	...
	for row: Dictionary in diagnostics.seat_states:
		...
		lines.append("Seat %s  role=%s  form=%s  faction=%s" % [RoleSession.SEAT_NUMERALS[row.seat - 1], state.assigned_role_id, state.form_id, state.faction_id])
		lines.append("  life=%s reveal=%s defeat=%s connected=%s objectives=%s uses=%s" % [state.lifecycle, state.revealed, state.defeated, state.connected, state.objective_refs, state.uses])
	return "\n".join(lines)
...
func _privacy_page(diagnostics: Dictionary) -> String:
	...
	return "\n".join(PackedStringArray([
		...
		"passed=%s  public_or_director_leaks=%s" % [report.passed, report.public_or_director_leaks],
		"unauthorized_seat_leaks=%s" % [report.unauthorized_seat_leaks],
		"Director-safe allowlisted aggregate output: %s" % JSON.stringify(diagnostics.director_safe_signals),
...
static func calculate_panel_rect(viewport_size: Vector2, safe_margin: int) -> Rect2:
	...
	var desired := Vector2(minf(PANEL_SIZE.x, safe.size.x - 20.0), minf(PANEL_SIZE.y, safe.size.y - 20.0))
