...
func handle_private_input(session: RoleSession, seat_number: int, confirm: bool, cancel: bool) -> bool:
	if _view_model.get("kind", "") != "seat_private" or _view_model.get("authorized_seat", 0) != seat_number or (not confirm and not cancel):
		return false
...
func _present_public(view: Dictionary, title: String) -> void:
	...
	lines.append("%s  •  r%d%s" % [view.mode_label, view.revision, "  •  AUTHORED SAFE FALLBACK" if view.fallback_active else ""])
	lines.append("Every row combines numeral, shape, count mark, symbol, pattern, text, and connection state.")
	lines.append(view.afterlife_notice)
	for seat: Dictionary in view.seats:
		lines.append("%s  %-9s  mark x%d  %s %s / %s  •  %s  •  %s" % [
			seat.numeral, seat.shape, seat.seat, seat.identity_symbol, seat.identity_label,
			seat.faction_label, seat.status, seat.connection,
		])
		if not seat.legal_actions.is_empty():
			lines.append("    LEGAL: %s" % ", ".join(seat.legal_actions))
	if not view.get("fallback_message", "").is_empty(): lines.append("SAFE FALLBACK: %s" % view.fallback_message)
	var history: Array = view.get("public_history", [])
	if not history.is_empty(): lines.append("LATEST: %s" % history[-1].message)
	_rendered_text = "\n".join(lines)
	...
	_view_model = {"kind": "public", "essential_lines": lines.size(), "essential_content_fits": lines.size() <= MAX_PLAYER_LINES, "contains_private_ids": false, "safe_margin": _safe_margin}

func _present_private(view: Dictionary, title: String) -> void:
	_backdrop.color = Color(0.015, 0.02, 0.035, 0.98)
	_backdrop.visible = true
	_title.text = "%s  •  SEAT %s ONLY" % [title, RoleSession.SEAT_NUMERALS[view.authorized_seat - 1]]
	...
	lines.append("")
	lines.append("Acknowledge, close, then restore the public view. Companion devices remain future work.")
	_rendered_text = "\n".join(lines)
	_body.text = _rendered_text
	_footer.text = "PRIVATE REVEAL  •  A / SPACE ACKNOWLEDGE  •  B / ESC CLOSE SAFELY"
	_view_model = {"kind": "seat_private", "authorized_seat": view.authorized_seat, "essential_lines": lines.size(), "essential_content_fits": lines.size() <= MAX_PLAYER_LINES, "shared_screen_obscured": true, "safe_margin": _safe_margin}
...
func _present_outcome(view: Dictionary, title: String) -> void:
	...
	for faction: Dictionary in outcome.get("factions", []):
		lines.append("%s %s  / %s  •  %s" % [faction.symbol, faction.label, faction.pattern, faction.result.to_upper()])
	lines.append("INDIVIDUAL SEATS")
	for seat: Dictionary in outcome.get("seats", []):
		lines.append("Seat %s  •  %s  •  %s" % [seat.numeral, seat.result.to_upper(), "; ".join(seat.objectives)])
	_rendered_text = "\n".join(lines)
	_body.text = _rendered_text
	_footer.text = "MULTIPLE WINNERS SUPPORTED  •  faction + individual + partial + mixed results"
	_view_model = {"kind": "outcome", "essential_lines": lines.size(), "essential_content_fits": lines.size() <= MAX_PLAYER_LINES, "safe_margin": _safe_margin}
...
static func calculate_panel_rect(viewport_size: Vector2, safe_margin: int, private_mode: bool = false) -> Rect2:
	var safe := Rect2(Vector2(safe_margin, safe_margin), viewport_size - Vector2(safe_margin, safe_margin) * 2.0)
