...
static func apply(decision: Dictionary, rules: RulesSession, board: BoardState) -> Dictionary:
	if (
		decision.get("decision_version") != DirectorRuntime.DECISION_VERSION
		or not decision.get("proposal") is Dictionary
	):
		return {
			"accepted": false,
			"reason": "malformed_director_proposal",
			"authority": "none",
	...
	match proposal.get("type", ""):
		...
		"rules_effects":
			...
			for effect: Dictionary in effects:
				if (
					target_seat > 0
					and not effect.has("seat")
					and (
						effect.get("type", "")
						in ["add_item", "remove_item", "draw_card", "grant_card"]
					)
				):
		...
		"presentation":
			...
			result = {
				"accepted": true,
				"reason": "presentation_cue_accepted",
				"presentation": proposal.duplicate(true)
			}
		_:
	...
	if not result.get("accepted", false):
		if rules.to_snapshot() != rules_before or board.to_snapshot() != board_before:
			return {
				"accepted": false,
				"reason": "rejected_proposal_changed_state",
				"authority": authority,
				"downstream_revision": -1
			}
	return {
		...
		(
			board.revision
			if authority == "BoardState"
			else rules.history().size() if authority == "RulesSession" else -1
		),
