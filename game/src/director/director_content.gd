...
const VALID_METRICS: PackedStringArray = [
	"progress", "failure_pressure", "resource_pressure", "hazard_pressure", "group_spread",
	"stalled_steps", "prompt_latency", "pass_frequency", "participation_imbalance", "rejected_actions",
]
const VALID_CATEGORIES: PackedStringArray = ["pressure", "relief", "hint", "board", "event", "ambient", "no_op"]
const VALID_CONDITIONS: PackedStringArray = ["metric_at_least", "metric_at_most", "rules_flag", "board_hazard_below", "always"]
const VALID_TARGET_SCOPES: PackedStringArray = ["none", "active_any", "active_negative"]
const VALID_BUDGETS: PackedStringArray = ["pressure", "relief", "clue", "scarcity", "ambient", "intervention"]
const VALID_PAYLOADS: PackedStringArray = ["queue_event", "rules_effects", "board_mutation", "presentation", "no_op"]
const VALID_TAGS: PackedStringArray = [
	"pressure", "relief", "clue", "scarcity", "ambient", "board", "event", "recovery",
	"light", "hazard", "nudge", "breath", "host", "sound", "gentle", "dramatic",
]
...
func validate(rules_content: RulesContent = null, board_definition: BoardDefinition = null) -> PackedStringArray:
	...
	for profile: Dictionary in profiles:
		...
		for candidate: Dictionary in candidates:
			...
			legal_fallback = allowed.is_empty() or tags.any(func(tag: Variant) -> bool: return allowed.has(tag))
			legal_fallback = legal_fallback and not tags.any(func(tag: Variant) -> bool: return denied.has(tag))
		if not legal_fallback:
			failures.append("profile %s cannot select legal no-op fallback" % profile.get("id", "?"))
	return failures
...
func _validate_profile(profile: Dictionary, seen: Dictionary, failures: PackedStringArray) -> void:
	...
	if not _valid_id(profile_id) or not profile.get("version") is int or profile.get("version", 0) < 1:
	...
	if not curve is Array or curve.is_empty():
		...
		for point: Variant in curve:
			if not point is Dictionary or not point.get("progress") is int or not point.get("low") is int or not point.get("high") is int or not point.get("act") is String:
				failures.append("invalid pacing curve %s" % profile_id)
				break
			if point.progress <= prior_progress or point.progress < 0 or point.progress > 100 or point.low < 0 or point.high > 100 or point.low > point.high or not _valid_id(point.act):
	...
	if not weights is Dictionary:
		...
		for metric: Variant in weights:
			if not metric is String or not VALID_METRICS.has(metric) or not weights[metric] is int or absi(weights[metric]) > 20:
	...
	if not bounds is Dictionary:
		...
		for metric: String in VALID_METRICS:
			...
			if not bound is Array or bound.size() != 2 or not bound[0] is int or not bound[1] is int or bound[0] >= bound[1]:
	...
	for field: String in ["global_cooldown", "tag_cooldown", "repetition_window", "target_window", "recovery_window", "min_spacing", "max_spacing", "max_chain", "max_retries", "max_targets_per_window", "pressure_window", "max_pressure_per_window"]:
	...
	if profile.get("min_spacing", 0) > profile.get("max_spacing", 0) or profile.get("max_chain", 0) < 1 or profile.get("max_retries", 0) < 1:
		failures.append("impossible spacing or retry limits %s" % profile_id)
	if not profile.get("volatility") is int or profile.get("volatility", -1) < 0 or profile.get("volatility", 101) > 100:
	...
	for tag_field: String in ["allow_tags", "deny_tags"]:
		...
		if not tags is Array or tags.any(func(tag: Variant) -> bool: return not tag is String or not VALID_TAGS.has(tag)):
...
func _validate_candidate(candidate: Dictionary, seen: Dictionary, rules_content: RulesContent, board_definition: BoardDefinition, failures: PackedStringArray) -> void:
	var candidate_id: String = candidate.get("id", "")
	if not _valid_id(candidate_id) or not candidate.get("version") is int or candidate.get("version", 0) < 1:
	...
	if not tags is Array or tags.is_empty() or tags.any(func(tag: Variant) -> bool: return not tag is String or not VALID_TAGS.has(tag)):
	...
	if not conditions is Array:
		...
		for condition: Variant in conditions:
			if not condition is Dictionary or not VALID_CONDITIONS.has(condition.get("type", "")):
				...
				continue
			if condition.type.begins_with("metric_") and (not VALID_METRICS.has(condition.get("metric", "")) or not condition.get("value") is int or condition.value < 0 or condition.value > 100):
	...
	if not affinities is Dictionary:
		...
		for metric: Variant in affinities:
			if not metric is String or not VALID_METRICS.has(metric) or not affinities[metric] is int or absi(affinities[metric]) > 100:
	...
	if not VALID_BUDGETS.has(candidate.get("budget_kind", "")) or not candidate.get("budget_cost") is int or candidate.get("budget_cost", -1) < 0:
		failures.append("invalid candidate budget %s" % candidate_id)
	for field: String in ["cooldown", "repetition_window", "pressure_impact", "relief_impact", "tension_impact"]:
	...
	match payload.type:
		"queue_event":
			if rules_content == null or rules_content.event_by_id(payload.get("event_id", "")).is_empty():
		...
		"rules_effects":
			...
			if rules_content == null:
				...
				rules_content._validate_effects(payload.get("effects", []), {}, board_definition, effect_failures)
		...
		"board_mutation":
			if board_definition == null or not payload.get("mutation") is Dictionary:
				...
				if not probe._validate_mutation(payload.mutation).valid:
					failures.append("invalid candidate board reference %s" % candidate_id)
		"presentation":
			if not payload.get("cue") is String or payload.get("cue", "").is_empty() or not payload.get("reduced_motion_safe", false) is bool:
