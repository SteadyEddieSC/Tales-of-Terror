...
static func calculate_frame(positions: Array[Vector2], viewport_size: Vector2, room_bounds: Rect2) -> Dictionary:
	if positions.is_empty():
		return {"center": room_bounds.get_center(), "zoom": 1.0, "separation": SeparationState.NORMAL}
	...
	var zoom: float = minf(viewport_size.x / maxf(required_size.x, 1.0), viewport_size.y / maxf(required_size.y, 1.0))
	zoom = clampf(zoom, MIN_ZOOM, MAX_ZOOM)
	var half_view: Vector2 = viewport_size * 0.5 / zoom
	var center: Vector2 = group_bounds.get_center()
	center.x = clampf(center.x, room_bounds.position.x + half_view.x, room_bounds.end.x - half_view.x)
	center.y = clampf(center.y, room_bounds.position.y + half_view.y, room_bounds.end.y - half_view.y)
...
static func movement_resistance(pawn_position: Vector2, input_vector: Vector2, group_center: Vector2) -> float:
...
static func _maximum_pair_distance(positions: Array[Vector2]) -> float:
	...
	for first: int in positions.size():
		for second: int in range(first + 1, positions.size()):
			maximum_distance = maxf(maximum_distance, positions[first].distance_to(positions[second]))
