...
func update_focus(pawns: Array[PawnState]) -> void:
	...
	for pawn: PawnState in pawns:
		...
		if (
			not nearest.is_empty()
			and (not focus_owners.has(nearest) or pawn.seat_number < focus_owners[nearest])
		):
			focus_owners[nearest] = pawn.seat_number
	for interactable_id: String in focus_owners:
		(_interactables[interactable_id] as SandboxInteractable).set_focused(
			focus_owners[interactable_id]
		)
...
func resolve_pending() -> void:
	...
	for winner: Dictionary in InteractionResolver.resolve_requests(_pending):
		var interactable: SandboxInteractable = (
			_interactables.get(winner.interactable_id) as SandboxInteractable
		)
