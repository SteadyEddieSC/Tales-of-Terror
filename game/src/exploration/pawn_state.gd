...
func _init(
	p_seat_number: int, p_device_id: int, p_identity: String, spawn_position: Vector2
) -> void:
...
func clamp_to_bounds(bounds: Rect2) -> void:
	position.x = clampf(
		position.x, bounds.position.x + COLLISION_RADIUS, bounds.end.x - COLLISION_RADIUS
	)
	position.y = clampf(
		position.y, bounds.position.y + COLLISION_RADIUS, bounds.end.y - COLLISION_RADIUS
	)
