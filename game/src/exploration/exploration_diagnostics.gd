...
func update_snapshot(
	pawns: Array[PawnState],
	camera: SharedCameraCoordinator,
	...
	var lines: PackedStringArray = [
		...
		(
			"Camera target %s  zoom %.2f  %s"
			% [
				camera.target_position.round(),
				camera.target_zoom,
				SharedCameraPolicy.state_label(camera.separation_state)
			]
		),
	]
	for pawn: PawnState in pawns:
		var board_space: String = (
			board_state.space_for_seat(pawn.seat_number) if board_state != null else "—"
		)
		(
			lines
			. append(
				(
					"Seat %s → dev %d  pos %s  input %s  space %s  focus %s  %s"
					% [
						_roman(pawn.seat_number),
						pawn.device_id,
						pawn.position.round(),
						pawn.input_vector,
						board_space,
						pawn.nearby_interactable,
						"CONNECTED" if pawn.connected else "RESERVED",
					]
				)
			)
		)
	if board_state != null:
		lines.append(
			(
				"BOARD %s v%d  REVISION %d"
				% [
					board_state.definition.board_id,
					board_state.definition.board_version,
					board_state.revision
				]
			)
		)
	...
	if rules_session != null:
		...
		lines.append(
			(
				"RULES %s  SEED %d/#%d  ROUND %d %s r%d"
				% [
					rules.session,
					rules.seed,
					rules.rng_counter,
					rules.round,
					rules.phase,
					rules.phase_revision
				]
			)
		)
		lines.append(
			(
				"READY %s PASS %s  EVENT %s  QUEUE %s"
				% [rules.ready, rules.passed, rules.current_event, rules.event_queue]
			)
		)
		lines.append(
			(
				"CARDS deck=%d discard=%d exhaust=%d  INVENTORY %s"
				% [rules.deck, rules.discard, rules.exhausted, rules.inventory]
			)
		)
		lines.append(
			(
				"PROMPT %s  VOTE %s  CHECK %s"
				% [
					rules.prompt.get("id", "—"),
					rules.vote.get("id", "—"),
					rules.check.get("outcome", "—")
				]
			)
		)
	_label.text = "\n".join(lines)
...
func _connector_summary(board_state: BoardState) -> PackedStringArray:
	...
	return PackedStringArray(
		["CONNECTORS A  " + " | ".join(first), "CONNECTORS B  " + " | ".join(second)]
	)
...
func _space_value_summary(board_state: BoardState) -> PackedStringArray:
	...
	return PackedStringArray(
		[
			"HAZARDS  " + ("none" if hazards.is_empty() else " | ".join(hazards)),
			"FEATURES  " + ("none" if features.is_empty() else " | ".join(features))
		]
	)
...
func _history_summary(board_state: BoardState) -> PackedStringArray:
	...
	for entry: Dictionary in board_state.recent_history(3):
		...
		lines.append(
			ellipsize_to_width(line, INNER_TEXT_WIDTH, ThemeDB.fallback_font, DIAGNOSTIC_FONT_SIZE)
		)
	if lines.is_empty():
...
static func calculate_panel_rect(viewport_size: Vector2, safe_margin: int) -> Rect2:
	...
	return Rect2(
		position,
		Vector2(minf(PANEL_SIZE.x, viewport_size.x - position.x - safe_margin - 10), PANEL_SIZE.y)
	)


static func ellipsize_to_width(
	text: String, max_width: float, font: Font, font_size: int
) -> String:
