...
const VALID_TYPES: PackedStringArray = [REVEAL_SPACE, SET_CONNECTOR_STATE, SET_HAZARD, SET_FEATURE, SET_BLOCKER]
...
static func conflict_key(mutation: Dictionary) -> String:
	match mutation.get("type", ""):
		...
		SET_HAZARD, SET_FEATURE, SET_BLOCKER:
			return "%s:%s:%s" % [mutation.get("type", ""), mutation.get("space_id", ""), mutation.get("value_id", "")]
		_:
