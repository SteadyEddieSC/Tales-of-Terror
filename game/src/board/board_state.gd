	...
	for space: Dictionary in definition.spaces:
		...
		(
			...
			. append(
				{
					"id": space.get("id", ""),
					"label": space.get("name", space.get("id", "").capitalize()),
					"revealed": true,
					"occupants": occupants_in(space.get("id", "")),
					"hazard_count": state.get("hazards", []).size(),
					"feature_count": state.get("features", []).size(),
				}
			)
		)
...
func directly_connected(
	from_space: String, to_space: String, traversable_only: bool = false
) -> bool:
	for connector: Dictionary in definition.connectors:
		var direction_matches: bool = connector.from == from_space and connector.to == to_space
		var reverse_matches: bool = (
			not connector.get("one_way", false)
			and connector.from == to_space
			and connector.to == from_space
		)
...
func is_connector_traversable(
	connector_id: String, from_space: String = "", to_space: String = ""
) -> bool:
...
func crossing_is_blocked(from_space: String, to_space: String) -> bool:
	return (
		directly_connected(from_space, to_space, false)
		and not directly_connected(from_space, to_space, true)
	)
...
func mutation_disconnects_required(mutation: Dictionary) -> bool:
	...
	if (
		not _connector_states.has(connector_id)
		or not BoardDefinition.VALID_CONNECTOR_STATES.has(proposed_state)
	):
		return false
...
func apply_mutation(mutation: Dictionary, actor_seat: int = 0) -> Dictionary:
	...
	match mutation_type:
		...
		BoardMutation.SET_HAZARD:
			no_change = (
				_collection_has(_hazards[mutation.space_id], mutation.value_id) == mutation.active
			)
		BoardMutation.SET_FEATURE:
			no_change = (
				_collection_has(_features[mutation.space_id], mutation.value_id) == mutation.active
			)
		BoardMutation.SET_BLOCKER:
			no_change = (
				_collection_has(_blockers[mutation.space_id], mutation.value_id) == mutation.active
			)
	if no_change:
...
func apply_mutation_requests(requests: Array[Dictionary]) -> Array[Dictionary]:
	...
	ordered.sort_custom(
		func(a: Dictionary, b: Dictionary) -> bool:
			...
			return (
				BoardMutation.signature(a.get("mutation", {}))
				< BoardMutation.signature(b.get("mutation", {}))
			)
	)
...
func _validate_mutation(mutation: Dictionary) -> Dictionary:
	...
	match mutation_type:
		BoardMutation.REVEAL_SPACE:
			if (
				not _revealed.has(mutation.get("space_id", ""))
				or not mutation.get("revealed") is bool
			):
				return {"valid": false, "reason": "malformed_reveal_mutation"}
		BoardMutation.SET_CONNECTOR_STATE:
			if (
				not _connector_states.has(mutation.get("connector_id", ""))
				or not BoardDefinition.VALID_CONNECTOR_STATES.has(mutation.get("state", ""))
			):
		...
		BoardMutation.SET_HAZARD, BoardMutation.SET_FEATURE, BoardMutation.SET_BLOCKER:
			if (
				not _revealed.has(mutation.get("space_id", ""))
				or not _valid_value_id(mutation.get("value_id"))
				or not mutation.get("active") is bool
			):
...
func _parse_snapshot(snapshot: Dictionary) -> Dictionary:
	...
	for connector_id: String in expected_connectors:
		if (
			not connectors.has(connector_id)
	...
	for row_value: Variant in snapshot.occupancy:
		...
		if (
			not row.get("seat_number") is int
			or row.seat_number < 1
			or row.seat_number > SeatManager.MAX_SEATS
			or occupancy.has(row.seat_number)
		):
			return {"valid": false, "reason": "malformed_snapshot_occupancy"}
	...
	for entry_value: Variant in snapshot.history:
		if (
			...
			or not entry_value.get("revision") is int
			or not entry_value.get("type") is String
		):
			return {"valid": false, "reason": "malformed_snapshot_history"}
		if (
			entry_value.revision <= previous_history_revision
			or entry_value.revision > snapshot.revision
		):
			return {"valid": false, "reason": "malformed_snapshot_history"}
...
func _traversable_neighbors(space_id: String) -> PackedStringArray:
	...
	for connector: Dictionary in definition.connectors:
		if (
			connector.from == space_id
			and is_connector_traversable(connector.id, connector.from, connector.to)
		):
			neighbors.append(connector.to)
		if (
			connector.to == space_id
			and is_connector_traversable(connector.id, connector.to, connector.from)
		):
...
func _commit(type: String, payload: Dictionary, summary: String) -> void:
	...
	var entry: Dictionary = {
		"revision": revision, "type": type, "summary": summary, "payload": payload.duplicate(true)
	}
...
func _set_collection_value(
	store: Dictionary, space_id: String, value_id: String, active: bool
) -> void:
...
func _mutation_summary(mutation: Dictionary, actor_seat: int) -> String:
	...
	match mutation.type:
		BoardMutation.REVEAL_SPACE:
			return (
				"%s %s %s" % [actor, "revealed" if mutation.revealed else "hid", mutation.space_id]
			)
		BoardMutation.SET_CONNECTOR_STATE:
		...
		BoardMutation.SET_HAZARD, BoardMutation.SET_FEATURE, BoardMutation.SET_BLOCKER:
			return (
				...
				% [
					actor,
					"activated" if mutation.active else "cleared",
					mutation.space_id,
					mutation.value_id
				]
			)
