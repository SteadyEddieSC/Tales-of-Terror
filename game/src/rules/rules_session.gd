...
func _init(
	p_content: RulesContent = null,
	p_board_state: BoardState = null,
	p_seed: int = 1,
	seats: Array[int] = []
) -> void:
	if p_content != null:
...
func initialize(
	p_content: RulesContent, p_board_state: BoardState, p_seed: int, seats: Array[int]
) -> PackedStringArray:
	var failures: PackedStringArray = p_content.validate(
		p_board_state.definition if p_board_state != null else null
	)
...
func _contract_companion_public_view() -> Dictionary:
	...
	if not pending_prompt.is_empty():
		...
		if pending_prompt.get("scope", "all") == "single":
			...
			prompt_view = {
				"id": pending_prompt.get("id", ""),
				"revision": pending_prompt.get("revision", 0),
				"options": options,
				"eligible_seats": pending_prompt.get("eligible_seats", []).duplicate(),
	...
	return RulesContent.SessionData.normalize_json_numbers(
		{
			...
			"terminal": terminal_state != TerminalState.RUNNING,
			"terminal_reason": terminal_reason,
			"participating_seats": participating_seats.duplicate(),
			"connected_seats":
			participating_seats.filter(
				func(seat_number: int) -> bool: return seat_connection.get(seat_number, false)
			),
			"prompt": prompt_view,
		}
	)
...
func _contract_companion_seat_view(seat_number: int) -> Dictionary:
			...
			. append(
				{
					...
					"label": definition.get("label", definition.get("name", "Card")),
	...
	for item_id: String in inventory.get(seat_number, []):
		...
		inventory_view.append(
			{
				"id": item_id,
				"label": item.get("label", item.get("name", item_id.capitalize())),
...
func transition_phase(expected_revision: int = -1) -> Dictionary:
	...
	_record(
		"phase_changed",
		{"round": round_number, "phase": current_phase(), "revision": phase_revision}
	)
	return _accept()
...
func request_late_join(seat_number: int) -> Dictionary:
	if (
		seat_number < 1
		or seat_number > SeatManager.MAX_SEATS
		or participating_seats.has(seat_number)
		or pending_late_seats.has(seat_number)
	):
...
func mark_ready(
	seat_number: int, pass_turn: bool = false, expected_revision: int = -1
) -> Dictionary:
	if expected_revision >= 0 and expected_revision != phase_revision:
		return _reject("stale_phase_revision")
	if not participating_seats.has(seat_number):
	...
	if pass_turn:
		...
		passed_seats.sort()
	_record(
		"seat_passed" if pass_turn else "seat_ready",
		{"seat": seat_number, "phase_revision": phase_revision}
	)
	return _accept()


func open_prompt(
	definition: Dictionary, eligible_seats: Array[int], source_id: String = ""
) -> Dictionary:
	if not pending_prompt.is_empty():
	...
	_record(
		"prompt_opened",
		{"prompt_id": definition.id, "eligible_seats": eligible, "source_id": source_id}
	)
	return _accept()
...
func resolve_prompt() -> Dictionary:
	...
	var result: Dictionary = {
		...
		"winner": winners[0] if not winners.is_empty() else "pass",
		"counts": counts,
		"tie": winners.size() > 1,
		"responses": pending_prompt.responses.duplicate(true)
	}
	if result.winner != "pass":
		for option: Dictionary in pending_prompt.options:
			if option.id == result.winner and not option.get("effects", []).is_empty():
				var effects_result: Dictionary = apply_effect_bundle(
					option.effects, 0, pending_prompt.source_id
				)
...
func open_vote(definition: Dictionary, eligible_seats: Array[int]) -> Dictionary:
	...
	if result.accepted:
		active_vote = {
			"id": prompt.id,
			"rule": prompt.get("rule", "plurality"),
			"quorum": prompt.get("quorum", 1),
			"tie_policy": prompt.get("tie_policy", "stable_option_id"),
			"revision": phase_revision
		}
	return result
...
func resolve_check(
	definition: Dictionary, acting_seat: int = 0, source_id: String = ""
) -> Dictionary:
	...
	var modifier: int = (
		definition.get("modifier", 0) + counters.get(definition.get("modifier_counter", ""), 0)
	)
	var total: int = (
		rolls.reduce(func(sum: int, value: int) -> int: return sum + value, 0) + modifier
	)
...
func resolve_next_event() -> Dictionary:
	...
	if not event.get("effects", []).is_empty():
		...
		if not applied.accepted:
			return applied
	if event.has("check"):
		var check_result: Dictionary = resolve_check(
			event.check, event.get("acting_seat", 0), current_event_id
		)
		if not check_result.accepted:
			return check_result
		var outcome_effects: Array = event.get("outcome_effects", {}).get(
			check_result.result.outcome, []
		)
		if not outcome_effects.is_empty():
			var outcome_applied: Dictionary = apply_effect_bundle(
				outcome_effects, event.get("acting_seat", 0), current_event_id
			)
...
func play_card(seat_number: int, instance_id: String, targets: Array[int] = []) -> Dictionary:
	...
	if (
		targets.size() != target_count
		or targets.any(func(target: int) -> bool: return not participating_seats.has(target))
	):
	...
	if definition.policy != "retain":
		...
		if definition.policy == "exhaust":
			zone = exhausted_pile
		elif definition.policy == "remove":
			zone = removed_pile
...
func move_card_from_hand(seat_number: int, instance_id: String, destination: String) -> Dictionary:
	...
	elif destination == "exhausted":
		exhausted_pile.append(card)
	else:
		removed_pile.append(card)
	_record(
		"card_zone_changed",
		{"seat": seat_number, "instance_id": instance_id, "destination": destination}
	)
	return _accept()
...
func apply_effect_bundle(effects: Array, actor_seat: int = 0, source_id: String = "") -> Dictionary:
	...
	for effect: Dictionary in effects:
		...
		recent_effects.append(
			{"source_id": source_id, "actor_seat": actor_seat, "effect": effect.duplicate(true)}
		)
		if recent_effects.size() > 12:
			recent_effects.pop_front()
	_record(
		"effects_applied",
		{"source_id": source_id, "actor_seat": actor_seat, "count": effects.size()}
	)
	return _accept()
...
func _contract_to_snapshot() -> Dictionary:
	var snapshot: Dictionary = {
		...
		"event_chain_steps": event_chain_steps,
		"resolved_event_rounds": resolved_event_rounds.duplicate(true),
		...
		"history_sequence": _history_sequence,
...
func _contract_diagnostics_snapshot() -> Dictionary:
	return {
		...
		"exhausted": exhausted_pile.size(),
		"inventory": inventory,
		"vote": active_vote,
		"check": recent_check,
		"effects": recent_effects,
		"history": history(),
...
func _validate_check(definition: Dictionary) -> Dictionary:
	if (
		not definition.get("dice") is int
		or definition.dice < 1
		or definition.dice > 20
		or not definition.get("sides") is int
		or definition.sides < 2
		or definition.sides > 100
	):
		return {"valid": false, "reason": "malformed_check_definition"}
	if (
		not definition.get("modifier", 0) is int
		or not definition.get("advantage", 0) is int
		or absi(definition.get("advantage", 0)) > 1
	):
		return {"valid": false, "reason": "malformed_check_definition"}
...
func _validate_effect_bundle(effects: Array, actor_seat: int) -> Dictionary:
	var failures := PackedStringArray()
	content._validate_effects(
		effects, {}, board_state.definition if board_state != null else null, failures
	)
	if not failures.is_empty():
...
func _apply_effect(effect: Dictionary, actor_seat: int) -> void:
	match effect.type:
		"board_mutation":
			board_state.apply_mutation(effect.mutation, actor_seat)
		"set_counter":
			counters[effect.counter_id] = effect.value
		"add_counter":
			counters[effect.counter_id] = counters.get(effect.counter_id, 0) + effect.value
		"set_flag":
			flags[effect.flag_id] = effect.value
		"set_result":
			result_flags[effect.result_id] = effect.value
		"draw_card":
			draw_card(effect.get("seat", actor_seat), effect.get("count", 1))
		"grant_card":
			_grant_card(effect.get("seat", actor_seat), effect.card_id)
		"discard_card":
			move_card_from_hand(effect.get("seat", actor_seat), effect.instance_id, "discard")
		"exhaust_card":
			move_card_from_hand(effect.get("seat", actor_seat), effect.instance_id, "exhausted")
		"remove_card":
			move_card_from_hand(effect.get("seat", actor_seat), effect.instance_id, "removed")
		"add_item":
			inventory[effect.get("seat", actor_seat)].append(effect.item_id)
		"remove_item":
			inventory[effect.get("seat", actor_seat)].erase(effect.item_id)
		"queue_event":
			event_queue.append(effect.event_id)
		"history":
			_record("narrative", {"text": effect.get("text", "")})
	...
	for condition: Dictionary in conditions:
		match condition.type:
			"always":
				pass
			"flag_equals":
				if flags.get(condition.flag_id) != condition.value:
					return false
			"counter_at_least":
				if counters.get(condition.counter_id, 0) < condition.value:
					return false
			"seat_has_item":
				if not inventory.get(seat_number, []).has(condition.item_id):
					return false
			"phase_is":
				if current_phase() != condition.phase:
					return false
	return true
...
func _activate_late_seats() -> void:
	for seat_number: int in pending_late_seats:
		participating_seats.append(seat_number)
		seat_connection[seat_number] = true
		hands[seat_number] = []
		inventory[seat_number] = []
	participating_seats.sort()
