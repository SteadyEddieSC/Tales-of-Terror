...
func handle_navigation(seat_number: int, direction: int, confirm: bool, cancel: bool) -> bool:
	if (
		_session == null
		or _session.pending_prompt.is_empty()
		or not _session.pending_prompt.eligible_seats.has(seat_number)
	):
		return false
	...
	if cancel and _session.pending_prompt.get("allow_pass", false):
		var pass_result: Dictionary = _session.submit_response(
			seat_number, [], _session.pending_prompt.revision
		)
		refresh()
		return pass_result.accepted
	if confirm:
		var confirm_result: Dictionary = _session.submit_response(
			seat_number, [options[index].id], _session.pending_prompt.revision
		)
		refresh()
...
func get_view_model() -> Dictionary:
	...
	var model: Dictionary = {
		...
		(
			...
			if not _session.pending_prompt.is_empty()
				...
				"terminal"
				if _session.terminal_state != RulesSession.TerminalState.RUNNING
				else "summary"
			)
		),
		"presenter": {},
	...
	if not _last_presenter.is_empty():
		model.presenter = {
			"title": _last_presenter.get("title", "Event"), "body": _last_presenter.get("body", "")
		}
	if not _session.pending_prompt.is_empty():
...
func _prompt_view_model() -> Dictionary:
	...
	for option: Dictionary in prompt.options:
		options.append(
			{
				"id": option.id,
				"label": option.get("text", friendly_label(option.id)),
			...
			. append(
				{
					...
					"status_symbol":
					{"unresolved": "○", "locked": "✓", "pass": "—", "ineligible": "×"}[state],
...
func _card_view_model() -> Dictionary:
	...
	for seat_number: int in _session.participating_seats.slice(0, 3):
		...
		seats.append(
			{
				"numeral": ROMAN_NUMERALS[seat_number - 1],
				"hand_count": _session.hands[seat_number].size(),
				"items": item_names
			}
		)
	return {
		"deck_count": _session.draw_pile.size(),
		"discard_count": _session.discard_pile.size(),
		"exhaust_count": _session.exhausted_pile.size(),
		"seats": seats,
		"additional_seats": maxi(0, _session.participating_seats.size() - 3)
	}
...
func _compose_player_lines(model: Dictionary) -> PackedStringArray:
	...
	var lines := PackedStringArray(
		[
			...
			(
				"[b]%s  ◆  %s[/b]  •  %s"
				% [model.round_label, model.phase_label, model.terminal_label]
			),
		]
	)
	if model.mode == "prompt":
		...
		for seat: Dictionary in model.prompt.seat_states:
			...
			if seat.response_state == "unresolved":
				detail = "%s %s  •  choosing" % [seat.focus_symbol, seat.current_option_label]
			elif seat.response_state == "locked":
				detail = "%s  •  locked" % seat.current_option_label
			elif seat.response_state == "pass":
				detail = "Pass / Abstain  •  locked"
			lines.append(
				(
					"%s %s %s %s  %s"
					% [seat.status_symbol, seat.numeral, seat.symbol, seat.pattern, detail]
				)
			)
	else:
		...
		if not model.check.is_empty():
			lines.append(
				(
					"[b]CHECK ⚄ %s[/b]  %d → %s"
					% [model.check.label, model.check.total, model.check.outcome]
				)
			)
		if not model.cards.is_empty():
			lines.append(
				(
					"[b]CARDS ◫[/b] deck %d  discard %d  exhaust %d"
					% [model.cards.deck_count, model.cards.discard_count, model.cards.exhaust_count]
				)
			)
			for seat: Dictionary in model.cards.seats:
				lines.append(
					(
						"Seat %s  hand %d  items: %s"
						% [
							seat.numeral,
							seat.hand_count,
							", ".join(seat.items) if not seat.items.is_empty() else "None"
						]
					)
				)
			if model.cards.additional_seats > 0:
				lines.append(
					"+ %d more seats • details in diagnostics" % model.cards.additional_seats
				)
...
func _sync_prompt_selections() -> void:
	...
	var prompt_key: String = (
		"%s:%d"
		% [_session.pending_prompt.get("id", ""), _session.pending_prompt.get("revision", 0)]
	)
...
func _definition_display_name(stable_id: String) -> String:
	...
	if not event.is_empty():
		return event.get("title", friendly_label(stable_id))
	var card: Dictionary = _session.content.card_by_id(stable_id)
	if not card.is_empty():
		return card.get("name", friendly_label(stable_id))
