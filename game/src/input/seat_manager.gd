...
func find_reserved_seat(identity: String, device_id: int = -99) -> int:
	if identity.is_empty():
		return -1
	for index: int in MAX_SEATS:
		if _seats[index].state == SeatState.RESERVED and _seats[index].identity == identity and _seats[index].previous_device_id == device_id:
			return index
...
func _new_seat(index: int) -> Dictionary:
	return {"seat_number": index + 1, "state": SeatState.UNASSIGNED, "device_id": -99, "previous_device_id": -99, "identity": "", "device_name": "—", "last_action": "—"}
