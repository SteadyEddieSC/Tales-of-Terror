...
const SEAT_SHAPES: PackedStringArray = ["circle", "diamond", "triangle", "square", "star", "hexagon", "moon", "lantern"]
const SEAT_COLOR_NAMES: PackedStringArray = ["yellow", "blue", "orange", "ivory", "sky", "green", "plum", "amber"]
...
func public_payload(room_id: String, room_status: String, connected_clients: int) -> Dictionary:
	...
	return _json_copy({
		"view_version": VIEW_VERSION, "view_kind": "public_companion", "room_id": room_id,
		"room_status": room_status, "connected_clients": connected_clients, "seats": seats,
		"rules": _rules_session.companion_public_view(), "board": _board_state.companion_public_view(),
		"social": _role_session.public_view(), "director": _director_runtime.companion_public_view(),
	})
...
func seat_identity(seat_number: int) -> Dictionary:
	...
	return {
		"seat": seat_number, "numeral": TOKENS.player_symbols[index], "symbol": SEAT_SHAPES[index],
		"pattern": "%d segment%s" % [seat_number, "" if seat_number == 1 else "s"],
		"color_name": SEAT_COLOR_NAMES[index], "color_hex": TOKENS.player_colors[index].to_html(false),
	}
