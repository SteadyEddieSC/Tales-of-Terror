...
static func validate_envelope(value: Variant) -> Dictionary:
	...
	for key: Variant in envelope:
		if not key is String or not ENVELOPE_FIELDS.has(key):
			return {"accepted": false, "code": "malformed"}
	if (
		not envelope.has("protocol_version")
		or not envelope.protocol_version is int
	...
	if (
		not _valid_id(envelope.get("room_id"), 64)
		or not _valid_id(envelope.get("request_id"), MAX_REQUEST_ID_LENGTH)
	):
		return {"accepted": false, "code": "malformed"}
	for field: String in ["server_sequence", "authoritative_revision"]:
		if not envelope.get(field) is int or envelope[field] < 0:
			return {"accepted": false, "code": "malformed"}
	if (
		not envelope.get("seat_claim") is int
		or envelope.seat_claim < 0
		or envelope.seat_claim > SeatManager.MAX_SEATS
	):
		return {"accepted": false, "code": "malformed"}
	...
	if (
		not acknowledgement is String
		or (
			acknowledgement != ""
			and acknowledgement != "accepted"
			and not REJECTION_CODES.has(acknowledgement)
		)
	):
		return {"accepted": false, "code": "malformed"}
...
static func is_bounded_json(value: Variant, depth: int = 0) -> bool:
	...
	if value is Dictionary:
		...
		for key: Variant in value:
			if (
				not key is String
				or key.length() > MAX_STRING_LENGTH
				or not is_bounded_json(value[key], depth + 1)
			):
				return false
		return true
	return false
