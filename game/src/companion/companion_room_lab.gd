...
func present(bridge: CompanionBridge, stage: String, outcome: Dictionary = {}) -> void:
	...
	_title.text = "COMPANION ROOM LAB  •  %s" % stage.trim_prefix("companion_").replace("_", " ").to_upper()
	_status.text = "ROOM %s  •  IN-PROCESS FAKE TRANSPORT  •  PROTOCOL v%d / BRIDGE %s\nJOIN CODE  %s  •  CLIENTS %d / 8  •  PENDING %d  •  AUTHORITY r%d" % [
		diagnostics.room_state.to_upper(), diagnostics.protocol_version, diagnostics.bridge_version,
		diagnostics.join_code, diagnostics.connected_clients, diagnostics.pending_clients,
	...
	for claim: Dictionary in diagnostics.seat_claims:
		...
		claim_lines.append("%s  SEAT %s  %s  %s  •  %s" % [
			identity.symbol.to_upper(), identity.numeral, identity.pattern.to_upper(), identity.color_name.to_upper(),
			"CONNECTED" if claim.connected else "RESERVED FOR RESUME",
		])
	_claims.text = "STABLE-SEAT CLAIMS\n%s" % ("No approved companion claims." if claim_lines.is_empty() else "\n".join(claim_lines))
	_result.text = outcome.get("headline", "Host room is ready for explicit seat approval.")
	if outcome.has("detail"):
		_result.text += "\n" + outcome.detail
	_footer.text = "SANITIZED DIAGNOSTICS  •  %s  •  DUP %d  STALE %d  MALFORMED %d  UNAUTHORIZED %d\nNative Godot remains authoritative. The relay owns no rules. Local controller play remains available." % [
		diagnostics.privacy, diagnostics.counters.duplicate, diagnostics.counters.stale,
		diagnostics.counters.malformed, diagnostics.counters.unauthorized,
	]
	_combined.text = "\n".join([_title.text, _status.text, "", _claims.text, "", _result.text, "", _footer.text])
	visible = true
...
static func calculate_panel_rect(viewport_size: Vector2, safe_margin: int) -> Rect2:
	var safe := Rect2(Vector2(safe_margin, safe_margin), viewport_size - Vector2(safe_margin * 2, safe_margin * 2))
	var width: float = minf(820.0, safe.size.x)
	var height: float = minf(396.0, safe.size.y - 46.0)
	return Rect2(Vector2(safe.get_center().x - width * 0.5, safe.position.y + 40.0), Vector2(width, height))
