...
func initialize(
	...
	if (
		seat_manager == null
		or board_state == null
		or rules_session == null
		or director_runtime == null
		or role_session == null
	):
	...
	_view_builder = CompanionViewBuilder.new(
...
func create_room(p_room_id: String, p_join_code: String) -> Dictionary:
	if (
		room_open
		or CompanionProtocol.create_envelope(p_room_id, "room_created", "create_room").is_empty()
		or not _valid_join_code(p_join_code)
	):
	...
	var envelope: Dictionary = _envelope(
		"room_created", "create_room", {"join_code": join_code, "bridge_version": BRIDGE_VERSION}
	)
...
func close_room() -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"room_closed", "close_room", {"reason": "host_closed"}, 0, "accepted"
	)
...
func expire_room() -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"room_expired", "expire_room", {"reason": "host_missing_or_inactive"}, 0, "expired"
	)
	for client_id: String in _connected_clients:
		outbound_envelope.emit(client_id, envelope.duplicate(true))
	room_open = false
...
func request_join(client_id: String) -> Dictionary:
	...
	var payload: Dictionary = {
		...
		"pending_count": _pending_clients.size()
	}
...
func approve_claim(client_id: String, seat_number: int) -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"seat_claim_approved",
		"claim_%s" % CompanionProtocol.request_display(client_id),
		{
			"seat_identity": _view_builder.seat_identity(seat_number),
			"policy": "stable_seat_with_local_and_companion_input_surfaces"
		},
		seat_number,
		"accepted",
	)
...
func can_approve_claim(client_id: String, seat_number: int) -> bool:
	if (
		not _pending_clients.has(client_id)
		or not _rules_session.participating_seats.has(seat_number)
	):
		return false
	for claimed_client: String in _claims:
		if claimed_client != client_id and _claims[claimed_client] == seat_number:
...
func deny_claim(client_id: String) -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"seat_claim_rejected",
		"deny_%s" % CompanionProtocol.request_display(client_id),
		{},
		0,
		"unauthorized"
	)
...
func revoke_claim(client_id: String) -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"seat_claim_rejected",
		"revoke_%s" % CompanionProtocol.request_display(client_id),
		{"revoked": true},
		0,
		"revoked"
	)
...
func resume_client(client_id: String, seat_number: int) -> Dictionary:
	...
	var envelope: Dictionary = _envelope(
		"reconnect_resume",
		"resume_%s" % CompanionProtocol.request_display(client_id),
		{"restored": true},
		seat_number,
		"accepted"
	)
...
func receive_client_envelope(client_id: String, value: Variant) -> Dictionary:
	...
	var acknowledgement: Dictionary = _envelope(
		...
		envelope.request_id,
		{
			"resulting_revision": after_revision,
			"applied_once": true,
...
func authoritative_revision() -> int:
	if _rules_session == null:
		return 0
	return (
		_rules_session.authority_revision()
		+ _board_state.revision
		+ _role_session.revision
		+ _director_runtime.revision
	)


func public_view() -> Dictionary:
	return _view_builder.public_payload(
		room_id, "open" if room_open else "closed", connected_client_count()
	)
...
func diagnostics() -> Dictionary:
	...
	return {
		"protocol_version": CompanionProtocol.PROTOCOL_VERSION,
		"bridge_version": BRIDGE_VERSION,
		"room_state": "open" if room_open else "closed",
		"join_code": join_code,
		"connected_clients": connected_client_count(),
		"connected_client_records": _connected_clients.size(),
		"pending_clients": _pending_clients.size(),
		"seat_claims": claims,
		"client_sequence_entries": _last_client_sequences.size(),
		"ack_cache_entries": _ack_cache.size(),
		"ack_order_entries": _ack_order.size(),
		"sequence": _sequence,
		"queue_depth": 0,
		"last_authoritative_revision": authoritative_revision(),
		"stored_revision": _last_revision,
		"counters": _counters.duplicate(true),
		"history": _history.duplicate(true),
		"privacy": "NO CAPABILITIES OR PRIVATE PAYLOADS",
	}
...
func _emit_views(client_id: String) -> void:
	if not _claims.has(client_id):
		return
	var seat_number: int = _claims[client_id]
	var public_envelope: Dictionary = _envelope(
		"public_view_update",
		"public_%d" % _sequence,
		public_view(),
		0,
	...
	var private_envelope: Dictionary = _envelope(
		"seat_private_view_update",
		"private_%d" % _sequence,
		private_payload,
		seat_number,
		"",
		authoritative_revision()
	...
	if not private_payload.get("faction_private", {}).is_empty():
		outbound_envelope.emit(
			client_id,
			_envelope(
				"faction_private_view_update",
				"faction_%d" % _sequence,
				private_payload.faction_private,
				seat_number,
				"",
				authoritative_revision()
			)
...
func _cache_rejection(
	cache_key: String, client_id: String, request: Dictionary, code: String
) -> Dictionary:
	var envelope: Dictionary = _envelope(
		"rejection",
		request.get("request_id", "rejected"),
		{"refresh_required": code == "stale", "current_revision": authoritative_revision()},
		_claims.get(client_id, 0),
		code,
		authoritative_revision(),
	)
	...
	return _reject(
		code,
		request.get("message_type", "rejection"),
		request.get("request_id", "rejected"),
		_claims.get(client_id, 0),
		true,
		envelope
...
func _envelope(
	...
	return (
		...
		. create_envelope(
			...
			payload,
			_sequence,
			authoritative_revision() if revision < 0 else revision,
			seat_claim,
			acknowledgement,
		)
	)
		...
		. append(
			{
				...
				"request_display": CompanionProtocol.request_display(request_id),
				"result": result,
				"seat": seat_number,
				"before_revision": before_revision,
				"after_revision": after_revision,
...
func _authority_rejection_code(reason: String) -> String:
	...
	if "duplicate" in reason or "already" in reason:
		return "duplicate"
	if (
		"unauthorized" in reason
		or "wrong_owner" in reason
		or "not_available" in reason
		or "disconnected" in reason
	):
		return "unauthorized"
	return "malformed"
