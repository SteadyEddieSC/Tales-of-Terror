...
def collect_ids(root: Path, pattern: str, key: str) -> tuple[set[str], dict[str, dict[str, Any]]]:
    ids: set[str] = set()
    ...
    for path in paths:
        ...
        for entry in values:
            ...
            require(identifier not in ids, f"duplicate authoritative ID: {identifier}")
            ids.add(identifier)
...
def validate_index(data: dict[str, Any], root: Path = ROOT) -> None:
    ...
    require(isinstance(packages, list) and len(packages) == 7, "package index must contain P0.1 through P0.7")
    ...
    for index, package in enumerate(packages, start=1):
        ...
        require(set(package) == INDEX_PACKAGE_FIELDS, f"packages[{index - 1}] fields do not match contract")
        ...
        if release_id == "P0.1":
            require(package["merged_main_sha"] is None, "P0.1 legacy index entry must defer exact merge identity to PR #65")
        else:
        ...
        for path_index, path in enumerate(unique_text_list(package["primary_paths"], f"{release_id}.primary_paths", minimum=1)):
            validate_repo_path(root, path, f"{release_id}.primary_paths[{path_index}]")
        unique_text_list(package["governed_outputs"], f"{release_id}.governed_outputs", minimum=2)
        require(package["implementation_authorized"] is False, f"{release_id}: implementation authorization is prohibited")
        require(package["production_assets_approved"] is False, f"{release_id}: production asset approval is prohibited")
    ...
    current_main = text(data.get("current_protected_main_at_index_creation"), "current_protected_main_at_index_creation")
    require(SHA_PATTERN.fullmatch(current_main) is not None, "invalid protected-main SHA")
    require(current_main == packages[-1]["merged_main_sha"], "index creation SHA must equal the P0.7 protected-main squash")

    gates = data.get("known_external_gates")
    require(isinstance(gates, list) and gates, "known_external_gates must be a non-empty list")
    issue_numbers: set[int] = set()
    for gate in gates:
        require(isinstance(gate, dict) and set(gate) == {"issue", "summary"}, "invalid external-gate entry")
    ...
    require(issue_numbers == {7, 39, 44}, "external gates must include issues #7, #39, and #44")


def validate_traceability(data: dict[str, Any], root: Path = ROOT) -> None:
    require(data.get("traceability_kind") == "cross_media_preproduction_traceability", "unexpected traceability_kind")
    ...
    for unit_id, entry in access_entries.items():
        ...
        require(source_id not in voice_to_access, f"voice family has multiple accessibility units: {source_id}")
        voice_to_access[source_id] = unit_id

    entries = data.get("entries")
    require(isinstance(entries, list) and len(entries) >= 15, "traceability must contain at least fifteen concepts")
    concept_ids: list[str] = []
    titles: set[str] = set()

    for index, entry in enumerate(entries, start=1):
        require(isinstance(entry, dict), f"entries[{index - 1}] must be an object")
        require(set(entry) == TRACE_ENTRY_FIELDS, f"entries[{index - 1}] fields do not match contract")
        ...
        require(concept_id == f"DH-XM-{index:03d}", f"concept order is not sequential at {concept_id}")
        concept_ids.append(concept_id)
        title = text(entry["title"], f"{concept_id}.title", 8)
        require(title not in titles, f"duplicate concept title: {title}")
        titles.add(title)
        require(entry["criticality"] in {"foundational", "signature", "supporting"}, f"{concept_id}: invalid criticality")
        require(entry["human_validation_required"] is True, f"{concept_id}: human validation must remain required")
        require(entry["implementation_status"] == "preproduction_only", f"{concept_id}: implementation status must remain preproduction_only")
        text(entry["non_negotiable"], f"{concept_id}.non_negotiable", 50)

        design_paths = unique_text_list(entry["design_paths"], f"{concept_id}.design_paths", minimum=1)
        visual_paths = unique_text_list(entry["visual_paths"], f"{concept_id}.visual_paths", minimum=0)
        automation_paths = unique_text_list(entry["automation_paths"], f"{concept_id}.automation_paths", minimum=1)
        for field, values in (("design_paths", design_paths), ("visual_paths", visual_paths), ("automation_paths", automation_paths)):
            for path_index, path in enumerate(values):
                validate_repo_path(root, path, f"{concept_id}.{field}[{path_index}]")

        referenced_audio = unique_text_list(entry["audio_asset_ids"], f"{concept_id}.audio_asset_ids", minimum=0)
        referenced_music = unique_text_list(entry["music_asset_ids"], f"{concept_id}.music_asset_ids", minimum=0)
        referenced_voice = unique_text_list(entry["voice_family_ids"], f"{concept_id}.voice_family_ids", minimum=1)
        referenced_access = unique_text_list(entry["accessible_unit_ids"], f"{concept_id}.accessible_unit_ids", minimum=1)
        ...
        require(set(referenced_access) == expected_access, f"{concept_id}: voice and accessible-unit coverage do not match")
    ...
    require(required_titles.issubset(titles), "traceability omits a required foundational concept")
...
def validate_handoff(content: str) -> None:
    ...
    for phrase in required_phrases:
        require(phrase.lower() in lowered, f"Codex handoff missing required phrase: {phrase}")
    require("create `drowned_harbor` under the production Tale package directory" in content, "Codex handoff must explicitly prohibit a production Tale package")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Preproduction package traceability validation failed: {exc}", file=sys.stderr)
        return 1
    print("Validated seven merged preproduction packages, fifteen cross-media concepts, and the Codex return handoff")
    return 0
