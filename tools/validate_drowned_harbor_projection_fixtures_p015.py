#!/usr/bin/env python3
"""Validate inherited P0.14 fixtures against the P0.15 manifest progression."""
...
def validate_prototype_manifest(root: Path = ROOT) -> None:
    """Retain P0.14 fixture invariants while advancing issue #82 only."""
    ...
    inherited.require(
        ...
        "prototype manifest must record completed work issues #80 through #82",
    )
    inherited.require(
        manifest.get("future_work_issues") == [83, 84, 85, 86],
        "prototype manifest must leave issues #83 through #86 as future work",
    )
    inherited.require(
...
def validate(root: Path = ROOT) -> tuple[int, int]:
    """Run the inherited fixture engine with only manifest progression replaced."""
...
def main() -> int:
    try:
        ...
        print(
            f"Drowned Harbor P0.15 projection compatibility validation failed: {exc}",
            file=sys.stderr,
        )
        return 1
    package = inherited.read_json(ROOT / inherited.PACKAGE_PATH)
    print(
        "Validated inherited P0.14 fixture engine for P0.15: "
