...
def main() -> int:
    ...
    if FIXTURE.is_file():
        ...
        if report.get("schema_version") != 2 or report.get("release") != "v0.1.8":
            failures.append("playtest report fixture version is not v0.1.8 schema 2")
    ...
    if 'config/version="v0.1.8"' not in project:
        failures.append("project must provide the single v0.1.8 release identity")
    repository_workflow = (
