...
spec = importlib.util.spec_from_file_location(
    "validator", ROOT / "tools/validate_post_dh_ux_status.py"
)
...
def main() -> int:
    cases: list[tuple[str, Mutation]] = [
        ("schema drift", lambda d: d.__setitem__("schema_version", 4)),
        ...
        ("active current issue created", lambda d: d["current_release"].update({"issue": 126, "release_id": "future"})),
        ("current release activated", lambda d: d["current_release"].__setitem__("activation_authorized", True)),
        ("successor issue selected", lambda d: d["recommended_next_release"].__setitem__("github_issue", 126)),
        ...
        ("rights falsely resolved", lambda d: d["pending_inputs"][0].__setitem__("state", "resolved")),
        ("rights blocker removed", lambda d: d.__setitem__("pending_inputs", [])),
        ("rights blocker implementation authorized", lambda d: d["pending_inputs"][0].__setitem__("implementation_authorized", True)),
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            raise AssertionError("stale UX-pending claim survived")
        ...
        for path in [validator.README, validator.INDEX, validator.ROADMAP]:
            ...
            text = text.replace("Rights and provenance", "Unspecified prerequisite")
            text = text.replace("rights and provenance", "unspecified prerequisite")
            text = text.replace("rights/provenance", "unspecified-prerequisite")
        ...
        try:
            ...
            raise AssertionError("missing rights/provenance blocker survived")

    print(f"Validated {len(cases) + 2} fail-closed post-DH-UX status mutations")
    return 0
