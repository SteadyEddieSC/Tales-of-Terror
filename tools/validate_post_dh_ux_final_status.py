...
DEPENDENCY_BASE='93b496f9fe6c268467dcbba53f7b1c4d716033bd'
DEPENDENCY_ALLOWED={'tools/test_validate_drowned_harbor_controlled_private_shield.py', 'package.json', 'tools/validate_drowned_harbor_high_water_transformation.py', 'tools/validate_drowned_harbor_controlled_private_shield.py', 'tools/test_validate_drowned_harbor_high_water_transformation.py', 'package-lock.json'}
...
def validate_status(s:dict[str,Any])->None:
 ...
 checks(s,[
 ('schema_version',10),('status_kind','post_prototype_project_status'),('protected_main',BASE),('protected_main_semantics','exact_reconciliation_starting_baseline'),('as_of_date','2026-09-09'),('playable_release','v0.1.9'),('pending_inputs',[]),('closed_unmerged_pull_requests',[32,154]),('unrelated_open_pull_requests',[142,143,144,145,158]),('human_evidence_claimed',False),('runtime_implementation_authorized',False),('ux_implementation_authorized',False),('visual_implementation_authorized',False),
 ...
 ('status_reconciliation',{'branch':BRANCH,'issue':155,'pull_request':156,'release_type':'documentation_and_governance_reconciliation','state':'candidate_for_independent_review'}),
...
def validate_changed_paths(repair_paths:set[str],reconciliation_paths:set[str])->None:
 need(repair_paths==DEPENDENCY_ALLOWED,'isolated dependency repair scope drift')
...
def validate_git()->None:
 ...
 # P0.23 fetches the PR base with depth=1 even after a full checkout. Restore
 # real ancestry before enforcing scope; a missing graph must never waive it.
 if subprocess.check_output(['git','rev-parse','--is-shallow-repository'],text=True).strip()=='true':
  subprocess.run(['git','fetch','--unshallow','origin',BRANCH],check=True)
