#!/usr/bin/env python3
"""Validate the Drowned Harbor development-only isolation boundary."""
...
def validate_manifest(manifest: dict[str, Any], root: Path = ROOT) -> None:
    require(
        ...
        "prototype manifest fields do not match the closed contract",
    )
    require(
        manifest["prototype_kind"] == "isolated_tale_prototype",
        "unexpected prototype kind",
    )
    require(manifest["schema_version"] == 1, "unsupported prototype schema")
    require(
        manifest["prototype_id"] == "drowned_harbor_dev_only",
        "prototype identity must remain unmistakably dev-only",
    )
    require(manifest["tale_id"] == "drowned_harbor", "unexpected design Tale ID")
    require(
        "Not Shipped" in manifest["display_name"],
        "display name must state that the prototype is not shipped",
    )
    require(
        manifest["status"] == "development_only_export_excluded",
        "prototype status must remain development-only and export-excluded",
    )
    require(
    ...
    require(
        ...
        == "res://data/tales/tale_catalog_v1.json",
        "prototype must reference the accepted production catalog",
    )
    require(
        manifest["production_catalog_sha256"] == EXPECTED_CATALOG_DIGEST,
        "prototype catalog identity drifted",
    )
    require(
        manifest["production_tale_id"] == "lantern_house_vertical_slice",
        "Lantern House must remain the production Tale",
    )
    for field in (
        "production_catalog_registered",
        "production_provider_registered",
    ...
    require(
        manifest["human_validation_required"] is True,
        "future human validation must remain required",
    )

    dependencies = manifest["dependencies"]
    require(
        isinstance(dependencies, dict)
        and set(dependencies) == EXPECTED_DEPENDENCY_FIELDS,
        "dependency fields do not match the closed contract",
    )
    require(
        all(value is False for value in dependencies.values()),
        (
            "prototype may not add network, Companion, credential, telemetry, "
            "cloud, or production-asset dependencies"
        ),
    )
    ...
    require(
        ...
        and set(export_policy) == EXPECTED_EXPORT_FIELDS,
        "export policy fields do not match the closed contract",
    )
    require(
        export_policy["required_exclusion_pattern"] == "tests/*",
        "prototype must stay behind the existing tests/* export exclusion",
    )
    require(
        export_policy["internal_windows_preset"] == "Internal Windows x86_64",
        "unexpected Windows export preset",
    )
    require(
        export_policy["internal_linux_preset"] == "Internal Linux x86_64",
        "unexpected Linux export preset",
    )
    require(
        export_policy["ordinary_exports_include_prototype"] is False,
        "ordinary exports may not include the prototype",
    )
    ...
    require(
        len(source_authorities) == len(set(source_authorities)),
        "source authorities contain duplicates",
    )
    for source_path in source_authorities:
        require(
            isinstance(source_path, str)
            and not source_path.startswith(("/", "http://", "https://")),
            "source authority must be repository-relative",
        )
        require(
            (root / source_path).is_file(),
            f"source authority does not exist: {source_path}",
        )
    ...
    for phrase in (
        ...
        require(
            phrase in notes,
            f"prototype notes missing boundary phrase: {phrase}",
        )
...
def validate_production_catalog(root: Path = ROOT) -> None:
    ...
    require(
        ...
        "unexpected production catalog kind",
    )
    require(
        catalog.get("schema_version") == 1
        and catalog.get("catalog_version") == 1,
        "unsupported production catalog version",
    )
    require(
    ...
    require(
        ...
        "production catalog must contain exactly one Tale",
    )
    require(
        entries[0].get("tale_id") == "lantern_house_vertical_slice",
        "only production Tale must remain Lantern House",
    )
    catalog_text = json.dumps(catalog, ensure_ascii=False).lower()
    require(
        "drowned_harbor" not in catalog_text,
        "production catalog may not reference Drowned Harbor",
    )
    require(
        "res://tests/" not in catalog_text,
...
def validate_export_boundary(root: Path = ROOT) -> None:
    ...
    require(
        preset_text.count('name="Internal Windows x86_64"') == 1,
        "Windows internal export preset missing or duplicated",
    )
    require(
        preset_text.count('name="Internal Linux x86_64"') == 1,
        "Linux internal export preset missing or duplicated",
    )
    require(
        preset_text.count("tests/*") == 2,
        "both export presets must exclude tests/*",
    )
    for filename in (
...
def validate_readme(root: Path = ROOT) -> None:
    ...
    for heading in (
        ...
        require(
            heading in readme,
            f"README missing required section: {heading}",
        )
    for phrase in (
        ...
        require(
            phrase.lower() in readme.lower(),
            f"README missing required status phrase: {phrase}",
        )
    require(
        "1–8" in readme or "1-8" in readme,
        "README must retain supported stable-seat range",
    )
    require(
        "The Underteller" in readme,
        "README must explain the provisional host",
    )
    require(
        "Living Board" in readme,
        "README must explain the Living Board concept",
    )
    require(
        "Restless" in readme,
        "README must explain meaningful play after defeat",
    )
