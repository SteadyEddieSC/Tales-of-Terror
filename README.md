...
3. Complete the v0.1.3 pilot infrastructure and Windows/Linux candidate-build framework with verified Godot 4.7.1 export templates, exact artifact identity, bounded facilitator materials, and blank human evidence.
...
v0.1.3 completes the pilot framework and candidate-build infrastructure only. No human pilot occurred and no manual pass is claimed. The future real household or remote-observed pilot belongs to issue #39, which must generate a fresh candidate from the then-current protected `main` when players become available. Ordinary development may continue after v0.1.3 merges rather than waiting for that deferred session.
