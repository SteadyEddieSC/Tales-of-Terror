...
| GDScript Toolkit 4.5.0 | PyPI package `gdtoolkit==4.5.0`; source tag commit `b7a4935fc6483d51837f7080598dad456f4f7645`; reviewed direct selection in `requirements-dev.in`; complete Python 3.11.9 resolution and distribution hashes in `requirements-dev.txt` | Local/CI `gdlint` and `gdformat --check`; not shipped with the game | MIT in `third_party/licenses/gdtoolkit-4.5.0-MIT.txt` | Not vendored; hash-enforced install includes exact transitive versions |
| pip-tools 7.6.0 | PyPI package `pip-tools==7.6.0`; Jazzband `pip-tools` release `7.6.0` | Local maintenance tool used with Python 3.11.9 to generate `requirements-dev.txt`; not installed in CI or shipped with the game | BSD-3-Clause upstream; no package source redistributed | Not vendored; exact version and generation command recorded in `docs/technical/Toolchain_and_Testing.md` |
