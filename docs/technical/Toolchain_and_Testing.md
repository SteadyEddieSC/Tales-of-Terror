...
v0.1.1 adds eight reviewed first-party scripts—four presentation/report runtime files, a standalone suite, one focused GUT script, the memory-writer test seam, and a synthetic capture fixture—raising the explicit inventory from 74 to 82 files. `playtest_readiness_test.gd` is a fail-closed workflow step. `tools/validate_playtest_readiness.py` verifies the report fixture schema, approved user-data destination, required test surfaces, and absence of reporting network APIs or forbidden serialized identity fields. Both quality gates remain zero-finding over the complete 82-file inventory.
