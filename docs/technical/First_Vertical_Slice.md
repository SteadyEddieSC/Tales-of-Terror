...
Manifest version 1 uses exact schemas for supported seats, terminal, ending, rematch, companion, and deterministic fixture policies. The five stage IDs, entry/completion conditions, and complete operation-type sequences are ordered and fixed for this bounded slice. The explicit sequence policy requires event queue/open/resolve ordering, prompt and vote boundaries, card/check/Director ordering, role transition before afterlife action, and outcome resolution only in Ending. Missing, duplicate, reordered, or stage-incompatible operations reject before session construction. Event/card/prompt/vote references resolve through `LanternHouseRulesContent`; check IDs and effect bundles use small explicit registries. Role selectors must identify authored roles whose referenced transition trigger or action tag is compatible.
