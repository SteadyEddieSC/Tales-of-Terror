...
- Resumable boundaries: operation zero rejects a transaction or pending prompt/vote. Nonzero operation indices accept only the exact v1 prompt or vote submit/resolve boundary. The policy reconstructs the authoritative pending definition and exactly matches scope, authored seat, title, complete ordered option dictionaries/text/symbols/effects, selection/pass policy, vote rule/quorum/tie/abstain policy, and source; revision, eligible seats, and responses are validated separately. Same-ID content substitutions, unknown keys, arbitrary indices, and mismatched prompt/vote/transaction state reject without changing the receiver. Real prompt and vote waits both restore and complete exactly once.
- Protected reset and Cancel: active play, a retained prompt checkpoint, and an open room containing a claim plus cached acknowledgement all reset to a visible clean title with no authorities or seats. The main integration additionally clears developer-lab mode, hides `InputDisplayLab`, destroys the sandbox, shows `VerticalSliceView` with `TERROR TURN`, and permits a fresh lobby join through a synchronous test seam. Confirmation Cancel retains the roster in lobby, exposes no authorities, passes snapshot coherence, and can confirm and initialize normally.
...
| Focused GUT | PASS, 16 tests / 80 assertions |
...
- A temporary focused GUT failure probe produced exit code 1 and a failing JUnit report (17 tests, 16 passing, 1 failing, 80/81 assertions). The probe script and generated report were removed; the restored suite passed 16 tests and 80 assertions.
